...
void MatNode::print() {
	if (isNum) {
		std::cout << "[\"" << "isNum\", \"num=" << num << "\"]" << "---";
	}
	else {
		std::cout << "[\"" << "isEle\", \"ele=";
		for (int i = 0; i < (int)block.size(); i++)
			std::cout << block[i].name << ",";
		std::cout << "\"]" << "---";
	}
}
...
Matter::Matter(std::string& _name) {
...
bool Matter::intoElement() {
	...
	if (!preProcess(tar)) {
#ifdef TSETCEI
		std::cout << "数据预处理失败！处理前为 \""
			<< name << "\" ；处理后为 \"" << tar << "\" 。" << std::endl;
#endif
		return 0;
	}
    int i = 0;
	std::vector<MatNode> matlist;
	if (!lexicalAnalysis(matlist, tar)) {
#ifdef TESTCEI
		std::cout << "词法分析失败！处理前为 \"" << name << "\" ，已处理的信息：" << std::endl;
		std::cout << "(int)matlist.size=" << (int)matlist.size() << ", matlist=";
	...
	matlist.clear();
#ifdef TESTCEI
	std::cout << "物质解析完成，M(" << name << ") = " << getRelativeMolecureMass() << " g/mol" << std::endl;
#endif
	return 1;
}
...
void Matter::printValence() {
	...
	for (i = 0; i < leneleList; i++)
		col = (col > eleList[i].name.length() ? col : eleList[i].name.length());
	for (i = 0; i < leneleList; i++)
		std::cout << name << " 的元素 " << std::setw(col) << eleList[i].name << " 平均价态 " << std::setw(8) << eleList[i].valence.p() << std::endl;
}
bool Matter::lexicalAnalysis(std::vector<MatNode>& matlist, std::string& tar) {
	Element eletmp;
	MatNode tmpnode;
	...
	for (i = 0; i < tarlen; i++) {
		...
		if (tar[i] >= '0' && tar[i] <= '9') {
			...
			if (iend < i) {
#ifdef TESTCEI
				std::cout << "遇到无法解析的字符串！在 \"" << tar << "\" 中。" << std::endl;
#endif
				return 0;
			}
			eletmp.amount = 1;
			...
			default:
#ifdef TESTCEI
				std::cout << "发现不合法字符！是 \"" << tar << "\" 中的第"
					<< i + 1 << "个字符 \"" << tar[i] << "\" 。" << std::endl;
#endif 
				return 0;
			}
			tmpnode.isNum = 1;
...
void Matter::furtherLexicalAnalysis(std::vector<MatNode>& matlist) {
	bool leaveloop = 1;
	int i = 0; int j = 0; int k = 0;
	while (1) {
		for (i = 0; i < (int)matlist.size(); i++)
			if (matlist[i].isNum == 1 && matlist[i].num < 0) {
				leaveloop = 0; break;
			}
...
bool Matter::CalaBlock(std::vector<MatNode>& matlist, int i, int j) {
	...
	while (i <= j) {
		...
		for (k = 0; k < (int)matlist[i].block.size(); k++) {
			...
			matlist[is].block.push_back(matlist[i].block[k]);
		}
		i++;
...
bool Matter::CallastBlock(std::vector<MatNode>& matlist) {
...
bool Matter::preProcess(std::string& tar) {
	...
	for (int i = 0; i < (int)tar.length() - 1; i++) {
		if (tar[i] == '.') {
            ...
            if (j == (int)tar.length())tar.push_back(')');
		}
	}
	deleteAllMark(tar, ".");
	size_t pos = 0;
	...
	while (1) {
        ...
        int intpos = pos;
        while (intpos >= 0 && tar[intpos] >= '0' && tar[intpos] <= '9') {
            cq = cq + exp * (tar[intpos] - '0'); exp *= 10; tar.erase(intpos--, 1);
		}
		if (cq == 0)cq = 1;
	}
	q += cq; cq = 0; exp = 1;
...
int Matter::getEleAmount(std::string& _element) {
	for (int i = 0; i < (int)eleList.size(); i++)
		if (_element == eleList[i].name)
			return eleList[i].amount;
	return 0;
}
int Matter::getEleIndex(std::string& _element) {
	for (int i = 0; i < (int)eleList.size(); i++)
		if (_element == eleList[i].name)
			return i;
	return -1;
