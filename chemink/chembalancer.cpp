...
namespace chemfunction {
    ...
    string Fraction::p() {
        ...
        i2a((int) fabs(a), tmpa, 10);
        i2a((int) fabs(b), tmpb, 10);
        if (a == 0)
            fout = "0";
        else if (b == 1) {
            if (a > 0) {
    ...
    void Matrix_row_transformation(int m[][MAXM], int &row, int &col) {
        //  [1,row]  [1,col]
        int i = 0, j = 0, k = 0;
        Fraction ratio = 0, tmp = 0;
        bool ok = 0, first_not_zero = 0;
    ...
    void Matter::printValence() {
        ...
        for (i = 0; i < leneleList; i++)
            col = (col > eleList[i].name.length() ? col : eleList[i].name.length());
        for (i = 0; i < leneleList; i++)
            cout << name << " 的元素 " << std::setw(col) << eleList[i].name << " 平均价态 " << std::setw(8)
                 << eleList[i].valence.p() << endl;
    }
            ...
            for (j = 0; j < (int) mRList[i].eleList.size(); j++) {
                for (i0 = 0; i0 < cp; i0++)    //  把该元素的化合价跟产物里面所有该元素的化合价进行比对
                    for (j0 = 0; j0 < (int) mPList[i0].eleList.size(); j0++)
                        if (mRList[i].eleList[j].name == mPList[i0].eleList[j0].name &&
                            mRList[i].eleList[j].valence != mPList[i0].eleList[j0].valence) {
                            mRList[i].eleList[j].Reduced = 1;
            ...
            for (j = 0; j < (int) mPList[i].eleList.size(); j++) {
                for (i0 = 0; i0 < cr; i0++)
                    for (j0 = 0; j0 < (int) mRList[i0].eleList.size(); j0++)
                        if (mPList[i].eleList[j].name == mRList[i0].eleList[j0].name &&
                            mPList[i].eleList[j].valence != mRList[i0].eleList[j0].valence) {
                            mPList[i].eleList[j].Reduced = 1;
                            goto BREAKPOINT2;
                        }
                BREAKPOINT2:;
            }
            ...
            for (j = 0; j < (int) mRList[i].eleList.size(); j++)
                for (i0 = 0; i0 < cp; i0++)
                    for (j0 = 0; j0 < (int) mPList[i0].eleList.size(); j0++)
                        if (mRList[i].eleList[j].name == mPList[i0].eleList[j0].name &&
                            mRList[i].eleList[j].valence != mPList[i0].eleList[j0].valence &&
                            mPList[i0].eleList[j0].Reduced == 1)
                            if ((Abs(mRList[i].eleList[j].finalValence - mRList[i].eleList[j].valence) >
                                 Abs(mPList[i0].eleList[j0].valence - mRList[i].eleList[j].valence)))
