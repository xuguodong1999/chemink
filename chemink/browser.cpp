...
Browser::Browser(QWidget *parent) : QWidget(parent), clickedBtn(nullptr), eledata(nullptr) {
    ...
    leftBtn->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
    leftBtn->setArrowType(Qt::LeftArrow);
    rightBtn = new QToolButton(this);
    rightBtn->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
    rightBtn->setArrowType(Qt::RightArrow);
    molWindow = new GluMolWindow(this);
    molWindow->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
...
void Browser::handleBtnClickEvent(LabelButton *address) {
    ...
    if (SpellCorrector::getWordType(word) == SpellCorrector::EXPLICTCUT) {
        ...
        if (word[0] == 'C') {
            ...
            vector<int> input = {c};
            auto &isomerCounter = IsomerCounter::GetInstance();
            ref = std::move(isomerCounter.getIsomers(input));
